...
<div class="container py-8">
    ...
    <div class="mb-8">
        <div class="mb-6 flex flex-wrap justify-center space-x-2 space-x-reverse">
            ...
            <a href="{{ route('leaderboard.donation') }}" class="mb-2 rounded-lg bg-indigo-600 px-4 py-2 text-white">
                التبرعات
            </a>
        </div>
        ...
        @else
                ...
                <div class="flex flex-wrap items-end justify-center">
                    ...
                    @if(isset($topUsers[1]))
                        <div class="mx-3 text-center">
                            ...
                            <p class="text-sm text-gray-600">{{ number_format($topUsers[1]->total_donation_amount, 2) }} {{ config('app.currency', 'SAR') }}</p>
                        </div>
                    @endif
                    ...
                    @if(isset($topUsers[0]))
                        <div class="mx-3 -mb-4 text-center">
                            ...
                            <p class="text-sm font-semibold text-gray-600">{{ number_format($topUsers[0]->total_donation_amount, 2) }} {{ config('app.currency', 'SAR') }}</p>
                        </div>
                    @endif
                    ...
                    @if(isset($topUsers[2]))
                        <div class="mx-3 text-center">
                            ...
                            <p class="text-sm text-gray-600">{{ number_format($topUsers[2]->total_donation_amount, 2) }} {{ config('app.currency', 'SAR') }}</p>
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @foreach($topUsers as $index => $user)
                            <tr class="{{ auth()->check() && auth()->id() == $user->id ? 'bg-indigo-50' : '' }}">
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                    <span class="font-semibold text-indigo-600">{{ number_format($user->total_donation_amount, 2) }}</span> {{ config('app.currency', 'SAR') }}
                                </td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                    {{ $user->donation_count }} تبرع
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            @auth
                <div class="mt-8 rounded-lg bg-gray-50 p-4">
                    <h3 class="mb-2 text-lg font-semibold">ترتيبك في التبرعات</h3>
                    <div class="flex items-center justify-between">
                        ...
                        <div>
                            ...
                            <p>
                                <span class="font-semibold">إجمالي التبرعات:</span> 
                                {{ number_format($currentUserTotalDonation ?? auth()->user()->donations()->sum('amount'), 2) }} {{ config('app.currency', 'SAR') }}
                            </p>
                        </div>
                    </div>
                </div>
