...
<div class="container py-8">
    <div class="mb-8 text-center">
        <h1 class="mb-2 text-3xl font-bold text-gray-800">لوحة المتصدرين</h1>
        <p class="text-gray-600">أفضل المتطوعين والمساهمين في منصتنا</p>
    </div>

    <div class="mb-8">
        <div class="mb-6 flex flex-wrap justify-center space-x-2 space-x-reverse">
            <a href="{{ route('leaderboard.index') }}" class="mb-2 rounded-lg bg-indigo-600 px-4 py-2 text-white">
                النقاط الكلية
            </a>
            <a href="{{ route('leaderboard.participation') }}" class="mb-2 rounded-lg bg-gray-200 px-4 py-2 text-gray-700 hover:bg-gray-300">
                المشاركة في الحملات
            </a>
            ...
            <form action="{{ route('leaderboard.search') }}" method="GET" class="space-y-3">
                ...
                <div class="flex flex-wrap items-center justify-center gap-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('leaderboard.index') }}" class="rounded-lg {{ !request('user_type') ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-700' }} px-3 py-1 text-sm">
                            الكل
                        </a>
                        <a href="{{ route('leaderboard.index', ['user_type' => 'regular']) }}" class="rounded-lg {{ request('user_type') == 'regular' ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-700' }} px-3 py-1 text-sm">
                            أفراد
                        </a>
                        <a href="{{ route('leaderboard.index', ['user_type' => 'team']) }}" class="rounded-lg {{ request('user_type') == 'team' ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-700' }} px-3 py-1 text-sm">
                            فرق
                        </a>
                        <a href="{{ route('leaderboard.index', ['user_type' => 'organization']) }}" class="rounded-lg {{ request('user_type') == 'organization' ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-700' }} px-3 py-1 text-sm">
                            منظمات
                        </a>
                    </div>
                </div>
    ...
    <div class="mx-auto max-w-4xl">
        @if((isset($search) && $search && $topUsers->isEmpty()) || (isset($userType) && $userType && $topUsers->isEmpty()))
            <div class="mb-8 rounded-lg bg-yellow-50 p-4 text-center">
                ...
                <a href="{{ route('leaderboard.index') }}" class="mt-2 inline-block rounded-lg bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700">
                    عرض جميع المتطوعين
                </a>
            </div>
        @else
                ...
                <div class="flex flex-wrap items-end justify-center">
                    ...
                    @if(isset($topUsers[1]))
                        <div class="mx-3 text-center">
                            ...
                            <p class="text-sm text-gray-600">{{ $topUsers[1]->total_points }} نقطة</p>
                        </div>
                    @endif
                    ...
                    @if(isset($topUsers[0]))
                        <div class="mx-3 -mb-4 text-center">
                            ...
                            <p class="text-sm font-semibold text-gray-600">{{ $topUsers[0]->total_points }} نقطة</p>
                        </div>
                    @endif
                    ...
                    @if(isset($topUsers[2]))
                        <div class="mx-3 text-center">
                            ...
                            <p class="text-sm text-gray-600">{{ $topUsers[2]->total_points }} نقطة</p>
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @foreach($topUsers as $index => $user)
                            <tr class="{{ auth()->check() && auth()->id() == $user->id ? 'bg-indigo-50' : '' }}">
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                    <span class="font-semibold text-indigo-600">{{ $user->total_points }}</span> نقطة
                                </td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                    <div class="flex">
                                        @foreach($user->getFeaturedBadges(3) as $badge)
                                            <div class="mr-1 h-8 w-8 rounded-full" style="background-color: {{ $badge->color }};" title="{{ $badge->name }}">
                                                <div class="flex h-full w-full items-center justify-center text-white">
                                                    <i class="fa {{ $badge->icon }} fa-xs"></i>
                                                </div>
                                            </div>
                                        @endforeach

                                        @if(count($user->badges) > 3)
                                            <div class="mr-1 flex h-8 w-8 items-center justify-center rounded-full bg-gray-200 text-xs text-gray-600" title="المزيد من الشارات">
                                                +{{ count($user->badges) - 3 }}
                                            </div>
                                        @endif
                                    </div>
            ...
            @auth
                <div class="mt-8 rounded-lg bg-gray-50 p-4">
                    <h3 class="mb-2 text-lg font-semibold">ترتيبك</h3>
                    <div class="flex items-center justify-between">
                        ...
                        <div>
                            ...
                            <p><span class="font-semibold">النقاط:</span> {{ auth()->user()->getTotalPoints() }}</p>
                        </div>
                    </div>
                </div>
            @endauth
